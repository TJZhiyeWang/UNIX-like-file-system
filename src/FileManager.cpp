...
{
    this->m_FileSystem = &FSManager::Instance().GetFileSystem();
    this->m_InodeTable = &FSManager::Instance().GetInodeTable();

    /* 获取rootDirInode:0#inode */
    this->rootDirInode = FSManager::Instance().GetInodeTable().IGet(0);
    if(flag==true)
    {
        /* 初始化根节点目录项，包含.和.. */
        Buf *pBuf = FSManager::Instance().GetFileSystem().BAlloc();
        ...
        FSManager::Instance().GetBufferManager().Bwrite(pBuf);
        this->rootDirInode->i_flag |= Inode::IUPD;
        FSManager::Instance().GetInodeTable().IPut(this->rootDirInode);
    }


    /* 获取rootDirInode:0#inode */
    this->rootDirInode = FSManager::Instance().GetInodeTable().IGet(0);
    this->cdir = FSManager::Instance().GetInodeTable().IGet(0);
    this->pdir = FSManager::Instance().GetInodeTable().IGet(0);
    strcpy(this->curdir,"/");
}
    ...
    if(pInode!=NULL)//返回了父目录
    {
        ...
        while((count = this->fread(pInode,(char *)&tmpdir,32))!=0)
        {
            if(strcmp(tmpdir.m_name,iname)==0)//找到要删除的目录项
            {
                ...
                tmpnode->ITrunc();
                FSManager::Instance().GetFileSystem().IFree(tmpdir.m_inumber);
...
Inode* FileManager::NameI(const char *pathname, DirectorySearchMode mode, bool& flag)
{
    ...
    BufferManager& bufMgr = FSManager::Instance().GetBufferManager();
